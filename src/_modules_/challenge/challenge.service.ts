...
export class ChallengeService {
    ...
    async importActivitiesAfterJoinChallenge(
        ...
        let createChallengeActivityInput = []

        const activities = await Promise.all(activityIds.map(async item => {
            const activityId = item.id
            const {timezone} = item
            const activity = await this.activityService.findStravaActivity(`${activityId}`, access_token)
            const {
                distance,
                moving_time,
                elapsed_time, start_date_local, splits_metric,
            } = activity
            ...
            const formattedDate = moment(start_date_local).tz(timezone).utcOffset(0)
            const startDateFormatted = moment(formattedDate).startOf('day').format('YYYY-MM-DD 00:00:00');
        ...
        const challengeDailyActivitiesInput: Prisma.ChallengeDailyActivityCreateManyInput[] = Object.values(
